...
fn load_config(filename: String) -> Result<Config, Box<dyn std::error::Error>> {
...
pub fn recommend(
    ...
    let possible_recipes: Vec<Recipe> = enumerate::get_all_recipes(
        &config.ingredients.keys().collect::<Vec<_>>(),
        &config.processes,
        config.arcane_power,
        &config.alchemists,
        &config.market,
