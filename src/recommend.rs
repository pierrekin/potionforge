...
pub fn recommend(all_recipes: Vec<Recipe>) -> () {
    ...
    let _obj_coeffs = vec![1.0; all_recipes.len()];

    // The columns: a binary variable for each recipe with coeffecient 1.0.
    let cols = create_binary_columns(&mut model, all_recipes.len(), 1.);

    // The rows: constraints.
    let _rows = create_recipe_rows(&mut model, &cols, &all_recipes);
